class ZUSECL_PYC_BPC_SEL_POSTG_RUNID definition
  public
...
public section.
  ...
  protected section.
    ...
    constants mc_rel_post_doc_a type pyc_proc_step_template_id value 'ZHR_V2_REL_POST_DOC_A' ##NO_TEXT.
...
CLASS ZUSECL_PYC_BPC_SEL_POSTG_RUNID IMPLEMENTATION.
...
* Populate P_RUNID Selection
    loop at lt_runid into lv_runid.
      ls_sel_params-selname = 'P_RUNID' .
...
* Read Posting Jobs Headers
    ...
    data:
      ...
      lv_rele_step_id        type cl_pyc_proc_inst_aux=>ty_s_step_inst-step_id,
      lv_rele_step_sortfield type cl_pyc_proc_inst_aux=>ty_s_step_inst-si_sort_field.
...
* Identify Create Posting Step ID
    clear: ls_step_inst_enrich, lv_rele_step_id.
    read table lt_step_inst_enrich into ls_step_inst_enrich
      with key step_template_id	= mc_rel_post_doc_a.
    if sy-subrc eq 0.
      lv_rele_step_id = ls_step_inst_enrich-step_id.
    endif.

    if not lv_rele_step_id is initial.
* Read Batch Processing Component ID
      clear: lv_pyc_bpc_id.
      select single id into lv_pyc_bpc_id from  pyc_d_bpc
       where  pypi_id  = lv_proc_inst_id
         and  step_id  = lv_rele_step_id.
      if sy-subrc eq 0.
