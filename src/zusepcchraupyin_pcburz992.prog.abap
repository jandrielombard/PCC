...
form fuz_ter.
  ...
  constants: c_no_time_evaluation type p0007-zterf value '0'.     "MOD002++
...
*>>> Start of No Time Evaluation Change MOD002++
...
* Employee PDC Re Calculation Date
  if proto eq abap_true.
    message i001(zusehrpy_pcc_msg) with lt_p0003-bderr into lv_message.
...
* Employee Time Evaluation Variant
      if proto eq abap_true.
        message i002(zusehrpy_pcc_msg) with lv_program_var into lv_message.
        perform log_list_low in program h13plog0 tables $ptext using lv_message 1.
      endif.
...
* Employee new PDC Re Calculation Date
      if proto eq abap_true.
        message i003(zusehrpy_pcc_msg) with lt_p0003-bderr into lv_message.
        perform log_list_low in program h13plog0 tables $ptext using lv_message 1.
      endif.
...
form write_pcc_messages
  ...
  if not msg_num is initial.
    message id 'zusehrpy_pcc_msg' type 'S' number msg_num
          into data(msg_text)
          with param1 param2 param3 param4.

    move: 'S'      to messages-msgar.
    concatenate 'zusehrpy_pcc_msg' msg_num msg_text  into msg_text separated by '->'.
