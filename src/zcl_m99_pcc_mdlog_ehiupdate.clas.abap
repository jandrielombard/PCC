class ZCL_M99_PCC_MDLOG_EHIUPDATE definition
  public
  final
  create public .
...
* Payroll Run time
    data gt_pay_runtime  type zcl_m99_pcc_utilities=>tty_payrun_time.
...
CLASS ZCL_M99_PCC_MDLOG_EHIUPDATE IMPLEMENTATION.
...
* apply date/time conversion if the date/time entered is in AEST
    if iv_aest eq abap_true.
      call method zcl_m99_pcc_mdlog_ehiupdate=>convert_to_utc
        changing
...
* Populate Exclude User Names and infty from TVARVC
    call method zcl_m99_pcc_mdlog_ehiupdate=>get_ehiupd_exc_uname_infty
      importing
...
* Get process instance Payroll Area and Period
    ...
    if sy-subrc eq 0.
      " Read Period Details
      call method zcl_m99_pcc_mdlog_ehiupdate=>get_payroll_period_dates
        exporting
          iv_abkrs = gv_abkrs
          iv_inper = gv_inper
        importing
...
* Populate Exclude User Names
    ...
    if sy-subrc <> 0.
      ...
      select single admin_value into lv_admin_value
        from  pyd_d_atv where  admin_type   = zcl_m99_pcc_utilities=>gc_batch_user_type. "PYD_DAE_US
      if sy-subrc eq 0.
        clear lt_so.
...
* Populate Exclude Infotypes
    ...
    if sy-subrc <> 0.
      ...
      clear lt_so.
      lt_so = cl_pyd_fnd_aux=>set_so_fixed_value( iv_value = zcl_m99_pcc_utilities=>gc_it0003   ).
...
* Use Process Instance to read Process steps
    constants: mc_pty_program     type pyd_d_instp-par_type value 'PROGRAM',
               ...
               mc_report_name     type syrepid value 'ZHRAUREPY_PCC_MDLOG_EHIUPD00',
