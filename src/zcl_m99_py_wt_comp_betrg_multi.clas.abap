class zcl_m99_py_wt_comp_betrg_multi definition
  public
  ...
  protected section.
    ...
    constants mc_amt_low_01 type pyd_par_type value 'Z99_AMT_LOW_01' ##NO_TEXT.
    constants mc_amt_low_02 type pyd_par_type value 'Z99_AMT_LOW_02' ##NO_TEXT.
    constants mc_amt_low_03 type pyd_par_type value 'Z99_AMT_LOW_03' ##NO_TEXT.
    constants mc_amt_high_01 type pyd_par_type value 'Z99_AMT_HIGH_01' ##NO_TEXT.
    constants mc_amt_high_02 type pyd_par_type value 'Z99_AMT_HIGH_02' ##NO_TEXT.
    constants mc_amt_high_03 type pyd_par_type value 'Z99_AMT_HIGH_03' ##NO_TEXT.
    constants mc_itemid_multi type pyd_itemid value 'MULTI' ##NO_TEXT.
    ...
    data mv_amt_low_02 type maxbt .
    data mv_amt_low_03 type maxbt .
    data mv_amt_high_01 type maxbt .
    data mv_amt_high_02 type maxbt .
    data mv_amt_high_03 type maxbt .
...
CLASS ZCL_M99_PY_WT_COMP_BETRG_MULTI IMPLEMENTATION.
...
*---------------------------------------------------------------------*
    data: lt_all_result_count type tty_result_count,
          ls_all_result_count type ty_result_count.
...
* Collect the data
    data: lo_cloned_chkobj type ref to zcl_m99_py_wt_comp_betrg_multi.
...
* Build Result Table
    ...
    loop at lt_all_result_count into ls_all_result_count.
      ls_result-id = ls_all_result_count-dct_pernr.
...
* Amount Low Values
        mv_amt_low_01 =  cl_pyd_fnd_aux=>get_resp_fixed_value( iv_par_type = me->mc_amt_low_01
                                                               it_par      = mo_context->mt_par ).
        mv_amt_low_02 =  cl_pyd_fnd_aux=>get_resp_fixed_value( iv_par_type = me->mc_amt_low_02
                                                               it_par      = mo_context->mt_par ).
        mv_amt_low_03 =  cl_pyd_fnd_aux=>get_resp_fixed_value( iv_par_type = me->mc_amt_low_03
                                                               it_par      = mo_context->mt_par ).

* Amout High Values
        mv_amt_high_01 =  cl_pyd_fnd_aux=>get_resp_fixed_value( iv_par_type = me->mc_amt_high_01
                                                                it_par      = mo_context->mt_par ).
        mv_amt_high_02 =  cl_pyd_fnd_aux=>get_resp_fixed_value( iv_par_type = me->mc_amt_high_01
                                                                it_par      = mo_context->mt_par ).
        mv_amt_high_03 =  cl_pyd_fnd_aux=>get_resp_fixed_value( iv_par_type = me->mc_amt_high_01
                                                                it_par      = mo_context->mt_par ).
...
* Rebuild the list with out WT details
      ...
      endloop.
      delete lt_data_result where betrg not between mv_amt_low_01 and mv_amt_high_01.

* Delete Results not in between number high and number low
      refresh lt_result_count.
      loop at lt_data_result into ls_data_result.
        if ls_data_result-betrg between mv_amt_low_01 and mv_amt_high_01.
...
* Rebuild the list with out WT details
      ...
      loop at lt_data_result_2 into ls_data_result_2.
        ...
        collect ls_data_result_2 into lt_data_result.
      endloop.
      delete lt_data_result where betrg not between mv_amt_low_02 and mv_amt_high_02.

* Delete Results not in between number high and number low
      refresh lt_result_count.
      loop at lt_data_result into ls_data_result.
        if ls_data_result-betrg between mv_amt_low_02 and mv_amt_high_02.
          ls_result_count-dct_pernr = ls_data_result-dct_pernr.
          ls_result_count-count = 1.
          append ls_result_count to lt_result_count.
        endif.
...
* Rebuild the list with out WT details
      ...
      loop at lt_data_result_2 into ls_data_result_2.
        ...
        collect ls_data_result_2 into lt_data_result.
      endloop.
      delete lt_data_result where betrg not between mv_amt_low_03 and mv_amt_high_03.

* Delete Results not in between number high and number low
      refresh lt_result_count.
      loop at lt_data_result into ls_data_result.
        if ls_data_result-betrg between mv_amt_low_03 and mv_amt_high_03.
          ls_result_count-dct_pernr = ls_data_result-dct_pernr.
          ls_result_count-count = 1.
          append ls_result_count to lt_result_count.
        endif.
