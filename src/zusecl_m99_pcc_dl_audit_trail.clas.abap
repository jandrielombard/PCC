class ZUSECL_M99_PCC_DL_AUDIT_TRAIL definition
  public
  final
  create public .
...
public section.
  ...
  class-methods ENRICH_PROCESS_LOG
    importing
      !IT_PI_AL type ZUSEAUTTPY_PROC_INST_AL
    returning
      value(RT_PI_PH) type ZUSEAUTTPY_PROC_INST_PH .
  ...
  class-methods CONVERT_PROCESS_LOG_TSL
    importing
      !IT_PI_PH type ZUSEAUTTPY_PROC_INST_PH
    returning
      value(RT_PI_PH_EXPORT) type ZUSEAUTTPY_PROC_INST_PH_EXPORT .
  class-methods CONVERT_ALERT_LOG_TSL
    importing
      !IT_PI_ALH type PYC_T_PI_ALH_CSV
    returning
      value(RT_PI_ALH_EXPORT) type ZUSEAUTTPY_PI_ALH_CSV_EXPORT .
  class-methods CONVERT_ANALYTICS_LOG_TSL
    importing
      !IT_PI_KPI_AL type ZUSEAUTTPY_KPI_AL
    returning
      value(RT_PI_KPI_AL_EXPORT) type ZUSEAUTTPY_KPI_AL_EXPORT .
...
CLASS ZUSECL_M99_PCC_DL_AUDIT_TRAIL IMPLEMENTATION.
...
* Convert Process log Time to display format
    loop at it_pi_alh into ls_pi_alh.
      ...
      call method zusecl_m99_pcc_dl_audit_trail=>convert_timestamp_to_str
        exporting
          iv_time_stamp = lv_timestamp
          iv_tz         = zusecl_m99_pcc_dl_audit_trail=>c_utc
        receiving
...
*
    ...
    loop at it_pi_al into ls_pi_al.
      ...
      call method zusecl_m99_pcc_dl_audit_trail=>fill_message_parameter
        exporting
...
* Read attachments for the Process log and
        ...
        if lv_attachment_exists_fl eq abap_true.
          zusecl_m99_pcc_dl_audit_trail=>read_attachments( exporting it_filter     = lt_filter_facade
