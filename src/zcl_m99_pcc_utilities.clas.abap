class zcl_m99_pcc_utilities definition
  public
  final
  create public .
  ...
  public section.
    ...
    class-methods get_time_of_payroll_run
      ...
      exporting
        value(et_runtime) type zcl_m99_pcc_utilities=>tty_payrun_time .
    class-methods get_employees_payroll_runtime
      importing
        ...
        !iv_tpy      type hrdct_is_tpy
      exporting
        !et_runtime  type zcl_m99_pcc_utilities=>tty_payrun_time .
    class-methods get_pernr_payroll_runtime
      importing
        !iv_pernr       type pernr_d
        !io_res_context type ref to if_pyd_res_context
      exporting
        !et_pay_runtime type zcl_m99_pcc_utilities=>tty_payrun_time .
    ...
    class-methods get_asdir_file_list
      ...
      exporting
        ...
        !et_dir_list      type zhrau_tt_pcc_filelist
      exceptions
...
CLASS ZCL_M99_PCC_UTILITIES IMPLEMENTATION.
...
* Employee Range Table
    ...
    data: lt_runtime type zcl_m99_pcc_utilities=>tty_payrun_time.
...
* Process large number of Employee selection in batches
    if not it_pernr_so is initial.
      ...
      loop at it_pernr_so into data(ls_pernr).
        ...
        if lv_count eq 2000.
          refresh lt_runtime .
          call method zcl_m99_pcc_utilities=>get_time_of_payroll_run
            exporting
              iv_abkrs    = iv_abkrs
              iv_inper    = iv_inper
...
*        exporting
*          iv_msgid = zcl_m99_pcc_utilities=>gc_fi_recon_msgcls
