class ZUSECL_PYC_KPI_EE_IN_CORRECT definition
  public
  inheriting from ZUSECL_PYC_KPI_NUM_BASE
  create public
...
CLASS ZUSECL_PYC_KPI_EE_IN_CORRECT IMPLEMENTATION.
...
* get number of team members in correction
    ...
    IF lt_abkrs_so IS NOT INITIAL AND ls_t549q IS NOT INITIAL.
      SELECT COUNT( DISTINCT mcw~pernr ) INTO @ev_int4 FROM m_premw AS mcw
        ...
        WHERE mcw~koabr  =  @zusecl_pyc_kpi_ee_locked=>mc_lock_indicator-yes AND
